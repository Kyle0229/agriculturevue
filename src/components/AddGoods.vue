<template>
  <div class="hello" style="text-align: center">
    ...
    <el-form label-width="200px" style="margin-left: 100px">
      ...
      <el-form-item label="商品图片" style="width: 800px">
        ...
        <!--<input type="file" @change="getFile($event)" style="background-color: white" />-->
      ...
      <el-row style="width: 40%;margin-left: 100px">
        <el-button type="primary" plain @click="save()">确认提交</el-button>
      </el-row>
    </el-form>
  </div>
</template>
...
//      },
      ...
      save: function () {
          if(this.$route.path=='/admin/addgoods'){
            var url ="api/findShoper"
            axios.get(url).then(res=>{

              if (res.data!=null){
                this.goods.sid=res.data.sid;
              }
            })
          }else{
        ...
        axios.post("/api/save", this.goods).then(res => {
          if (res.data != null) {
              if(this.$router.match("/adminadmin")){
                this.$router.push('/adminadmin/sudgoods');
              }else{
